...
class Engine:
    def __init__(self, args):
        ...
        # Initialize wandb
        wandb.init(
            project="dialogue-act-crf",
            config=vars(args),
            name=f"{args.corpus}_run",
        )
    ...
    def train_epoch(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            loss = self.model(input_ids, attention_mask, chunk_lens, speaker_ids, topic_labels, labels=labels)
    ...
    def inference(self):
        self.model.load_state_dict(torch.load(f'ckp/model_{self.args.corpus}.pt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nfinetune', type=int, default=2,
                        help='num of the BERT layers to finetune. 0 means finetuning all layers')
